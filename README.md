...
## 一键安装
```bash
curl -sSL https://raw.githubusercontent.com/jrr996shujin-png/tavily-search-skill/main/install.sh | bash
```
...
OpenClaw 内置的 `web_search` 工具依赖 Brave Search API，申请需要信用卡。Tavily 提供免费套餐，注册即可使用，无需信用卡。

## 手动测试
```bash
python3 ~/.openclaw/workspace/skills/tavily-search/scripts/search.py "今天新闻"
```
